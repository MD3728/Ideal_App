...
namespace ServiceTest
{
  ...
  public partial class ServList : ServiceBase
  {
    ...
    private const int maxViolationCount = 7; // Maximum violation Amount (~35 seconds)
    ...
    // Updates firewall according to plans (occurs only on restart )
    public void UpdateFirewallRules()
    {
      ...
      pProcess.Start();
      pProcess.StandardInput.WriteLine("netsh advfirewall firewall show rule name=all");
      pProcess.StandardInput.WriteLine("exit");
      pProcess.StandardInput.Close();

      // Read output (all rules)
      ...
      {
        if ((cLine.StartsWith("Rule Name:"))&&(cLine.Contains("IdealBlock: ")))
        {
          string ruleName = cLine.Substring(10).Trim();
          Process.Start("netsh", $"advfirewall firewall delete rule name=\"{ruleName}\"");
        }
      }

      // Add updated firewall rules
